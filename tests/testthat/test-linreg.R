...
test_that("print() method works", {
  ...
  expect_output(linreg_mod$print(),"linreg\\(formula = Petal\\.Length ~ Sepal\\.Width \\+ Sepal\\.Length, data = \\.self\\$data\\)")
...
test_that("plot() method runs without error", {
  ...
  # Check that the plot() method runs without throwing an error
  expect_no_error(linreg_mod$plot())
})
